<div id="edit-modal-{{ $article->id }}" tabindex="1" aria-hidden="true" class="hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-[calc(100%-1rem)] max-h-full">
    <div class="relative p-4 w-full max-w-3xl max-h-full">
        ...
        <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
            ...
            <div class="flex items-center justify-between p-4 md:p-5 border-b rounded-t dark:border-gray-600">
                ...
                <button type="button" class="text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm w-8 h-8 ms-auto inline-flex justify-center items-center dark:hover:bg-gray-600 dark:hover:text-white" data-modal-toggle="edit-modal-{{ $article->id }}">
            ...
            <!-- Modal body -->
            <form class="" method="post" action="/dashboard/articles/{{ $article->id }}" enctype="multipart/form-data">
                @csrf
                @method('put')
                <div class="grid gap-4 mb-4 grid-cols-2 p-4 md:p-5">
                    ...
                    <div class="col-span-2 sm:col-span-1">
                        ...
                        <p class="mt-1 text-sm text-gray-500 dark:text-gray-300">1920x1080 Max:5MB</p>
                        @error('image')
                            <div class="text-red-500 text-sm mt-3">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="col-span-2">
                        ...
                        <textarea id="editor-edit-{{ $article->id }}" name="body" rows="4" class="block p-2.5 w-full text-sm text-gray-900 bg-gray-50 rounded-lg border border-gray-300 focus:ring-red-500 focus:border-red-500 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white dark:focus:ring-red-500 dark:focus:border-red-500" placeholder="Write product description here">{{ $article->body }}</textarea>
                        @error('body')
                            <div class="text-red-500 text-sm mt-3">{{ $message }}</div>
                        @enderror
                    </div>
