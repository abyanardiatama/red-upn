<div id="edit-user-modal-{{ Auth::user()->id }}" tabindex="1" aria-hidden="true" class="hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-[calc(100%-1rem)] max-h-full">
    <div class="relative p-4 w-full max-w-lg max-h-full">
        ...
        <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
            ...
            <div class="flex items-center justify-between p-4 md:p-5 border-b rounded-t dark:border-gray-600">
                ...
                <button type="button" class="text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm w-8 h-8 ms-auto inline-flex justify-center items-center dark:hover:bg-gray-600 dark:hover:text-white" data-modal-toggle="edit-user-modal-{{ Auth::user()->id }}">
            ...
            <form class="" method="post" action="/dashboard/users/{{ Auth::user()->id }}" enctype="multipart/form-data">
                ...
                <div class="grid gap-4 mb-4 grid-cols-2 p-4 md:p-5">
                    ...
                    <div class="col-span-2">
                        ...
                        <img src="{{ file_exists(public_path('storage/user_images/' . Auth::user()->image)) ? asset('storage/user_images/' . Auth::user()->image) : Auth::user()->image }}" alt="{{ Auth::user()->name }}" alt="{{ Auth::user()->name }}" class="w-32 h-32 object-cover mb-2">
                    ...
                    <div class="col-span-2">
                        ...
                        <p class="mt-1 text-sm text-gray-500 dark:text-gray-300">JPEG, PNG, JPG (RATIO 1:1 & 5MB).</p>
                        @error('image')
                            <div class="text-red-500 text-sm mt-3">{{ $message }}</div>
                        @enderror
                    </div>
